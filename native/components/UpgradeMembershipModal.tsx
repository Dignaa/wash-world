...
export default function UpgradeMembershipModal({
  ...
  useEffect(() => {
    ...
    const fetchTypes = async () => {
      try {
        ...
        const better = all.filter((t: MembershipType) => t.price > membership.membershipType.price)
        setTypes(better)
  ...
  const priceDifference = selectedType ? selectedType.price - membership.membershipType.price : 0

  const handleConfirm = async () => {
    if (!selectedType) return
    try {
      const res = await fetch(`${process.env.EXPO_PUBLIC_API_URL}user/${membership.user.id}/memberships/${membership.id}/upgrade`, {
        method: 'PATCH',
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={styles.overlay}>
        <View style={styles.container}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable
                ...
                style={[styles.item, selectedType?.id === item.id && styles.selected]}
              >
                <Text style={selectedType?.id === item.id ? { color: '#fff' } : undefined}>
          ...
          <Button title="Confirm and Pay" onPress={handleConfirm} disabled={priceDifference <= 0} />
...
const styles = StyleSheet.create({
  ...
  item: { padding: 10, borderRadius: 6, borderWidth: 1, borderColor: '#ccc', marginVertical: 5 },
